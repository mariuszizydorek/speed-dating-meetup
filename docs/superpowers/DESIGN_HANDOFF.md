...
- **Prepare** shows the three management sub-tabs — **01 Setup / 02 Schedule / 03 Print**. Active tab = primary text + 2px accent bottom-border; inactive = muted text, transparent border; hover raises row background one step.
...
- Behaviour: each project holds its own roster + params + plans + active plan index. Switching loads that project; the active project auto-saves on every param/roster/plan change. Library starts empty — use **New project** or the Setup empty-state demo roster button.
...
- **Icebreakers** editor: one empty text field per round (optional). Blank = no Run banner that round. **Fill defaults** seeds the built-in prompts; **Clear** empties all. Stored as `params.icebreakers`. Cosmetic only — does not invalidate the schedule.
...
- **Icebreaker banner** (idle/conversation/move): only when that round’s `params.icebreakers` entry is non-empty — "ICEBREAKER · ROUND k" (or "NEXT ICEBREAKER" during move) + the prompt.
...
- **Navigation**: space switch (Prepare / Run event); Prepare sub-tabs switch Setup/Schedule/Print; CTAs advance the flow (Generate→Schedule, Print materials→Print, Start event→Run). Prepare restores `lastPrep`.
...
- **Params** (`EventParams` from spec §6) + additions: `tagCfg { w, h, align, companyPos, nameSize }`; `breaks[]` as specified; `icebreakers?: (string|null)[]` (per-round, blank-able; cosmetic).
...
- **Sample data**: a built-in 40-person / 12-company demo roster with generated emails (Setup empty-state “Try with a 40-person demo roster”). No photography or illustration anywhere.
