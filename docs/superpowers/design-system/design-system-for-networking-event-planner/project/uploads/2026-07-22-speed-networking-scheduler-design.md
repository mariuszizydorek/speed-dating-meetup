...
export interface RunState {
  ...
  pausedFromPhase?: RunPhase;                     // when paused, phase to restore on resume
}
...
- On pause, store `pausedFromPhase` so Resume restores conversation / move / break correctly.
...
### 8.1 SetupPage
- **Roster panel**: drop-zone / file picker for `.xlsx` / `.csv`; preview table; inline row-level errors (missing name, duplicate name); manual add/edit/delete row. Columns are auto-detected as `Name` and `Company` (case-insensitive); no column-mapper UI in v1.
...
- Upload fixture CSV (`cypress/fixtures/roster-25.csv`, 25 people) → configure params → generate → check quality card → open Print → click each artifact download → start Run → advance a round. (Parser still supports xlsx; E2E uses CSV for a simple checked-in fixture.)
